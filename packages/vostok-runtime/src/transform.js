// @flow
// this module is adapted from https://github.com/facebook/jest/blob/master/packages/jest-resolve/src/transform.js
...
type Options = {| isInternalModule?: boolean |};

export const EVAL_RESULT_VARIABLE = "Object.<anonymous>";
...
const getCacheKey = (fileData: string, filename: Path, config: Config, instrument: boolean): string => {
  ...
  if (transformer && typeof transformer.getCacheKey === "function") {
    return transformer.getCacheKey(fileData, filename, configString, { instrument, watch: config.watch });
  } else {
    return crypto
      .createHash("md5")
      .update(fileData)
...
const getFileCachePath = (filename: Path, config: Config, content: string, instrument: boolean): Path => {
  const baseCacheDir = getCacheFilePath(config.cacheDirectory, "jest-transform-cache-" + config.name, VERSION);
  const cacheKey = getCacheKey(content, filename, config, instrument);
  // Create sub folders based on the cacheKey to avoid creating one
  // directory with many files.
  const cacheDir = path.join(baseCacheDir, cacheKey[0] + cacheKey[1]);
  const cachePath = path.join(cacheDir, path.basename(filename, path.extname(filename)) + "_" + cacheKey);
...
const instrumentFile = (content: string, filename: Path, config: Config): string => {
...
export const transformSource = (filename: Path, config: Config, content: string, instrument: boolean): string => {
  ...
  if (transform && shouldTransform(filename, config)) {
    result = transform.process(result, filename, config, { instrument, watch: config.watch });
  }
...
const transformAndBuildScript = (filename: Path, config: Config, options: ?Options, instrument: boolean): vm.Script => {
  const isInternalModule = !!(options && options.isInternalModule);
  const content = stripShebang(fs.readFileSync(filename, "utf8"));
  let wrappedResult;
  const willTransform = !isInternalModule && (shouldTransform(filename, config) || instrument);

  try {
    if (willTransform) {
      wrappedResult = wrap(transformSource(filename, config, content, instrument));
    } else {
...
const transform = (filename: Path, config: Config, options: Options): vm.Script => {
  const scriptCacheKey = getScriptCacheKey(filename, config, false);
  let script = cache.get(scriptCacheKey);
  if (script) {
    return script;
  } else {
    script = transformAndBuildScript(filename, config, options, false);
